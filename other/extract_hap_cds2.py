...
def load_genome_fasta(fasta_f:str) -> dict:
    '''
    Load the sequences from a target genome FASTA file.
    Args:
        fasta_f: (str) Path to input FASTA
    Returns:
        genome_dict: (dict) Dictionary of input sequences.
    '''
        ...
        for line in f:
            ...
            if line.startswith('>'):
                ...
                if name is not None:
                    ...
                    genome_dict[name] = chrom_sequence
...
def load_variants(vcf_f:str, annotations:dict, snps_only:bool=SNPS_ONLY) -> dict:
    '''
    Load the variants from an input VCF/BCF file and filter 
    to only retain those spanning the annotations.
    Args:
        vcf_f : (str) Path to input VCF/BCF
        annotations : (dict) Dictionary of Transcript objects
    Returns:
        variants : (dict) Dictionary of per-transcript variants
    '''
    print('\nLoading variants from the VCF/BCF file...')
    # Outputs
    variants = dict()
    n_variants = 0
    # Loop over the transcripts and coding sequences
    for i, ann in enumerate(annotations):
        transcript = annotations[ann]
        assert isinstance(transcript, Transcript)
        for cds in transcript.exons_l:
            assert isinstance(cds, CodingExon)
            # Initialize the output dictionary
            variants.setdefault(cds.id, list())
            # Extract only the variants that span each transcript
            bcft_cmd = ['bcftools', 'view', '-H']
            # Add conditional arguments
            if snps_only:
                bcft_cmd.append('--types')
                bcft_cmd.append('snps')
            bcft_cmd.append(f'{vcf_f}')
            bcft_cmd.append(f'{cds.chrom}:{cds.start}-{cds.stop}')
            # Run the command and capture the output
            process = subprocess.run(bcft_cmd, capture_output=True, check=True, text=True)
            vcf_records = process.stdout.strip('\n').split('\n')
            ...
            for record in vcf_records:
                ...
                genotypes = list()
                # First, where in the format field is GT located
                gt_idx = None
                for idx, fmt in enumerate(fields[8].split(':')):
                    if fmt == 'GT':
                        gt_idx = idx
                assert gt_idx is not None, f'Error: GT field not found in VCF at {chr}:{pos}'
                # Extract that index per sample
                for indv in fields[9:]:
                    gt = indv.split(':')[gt_idx]
                    alleles = re.split('[|/]', gt)
                    for allele in alleles:
                        if not allele.isnumeric():
                            allele = None
                        else:
                            allele = int(allele)
                        genotypes.append(allele)
                variant = VcfVariant(chrom, pos, ref, alts, genotypes)
                # Load into output
                variants[cds.id].append(variant)
                n_variants += 1
    # Report to log
    print(f'    Loaded {n_variants:,} variants from the input VCF/BCF.', flush=True)
    return variants
...
class VcfVariant:
    '''Variant record from a VCF/BCF file'''
    def __init__(self, chrom:str, position_bp:int,
                 ref_allele:str, alt_alleles:list,
                 genotypes:list):
        self.chr  = chrom
        self.pos  = position_bp
        self.geno = genotypes
        # Process alleles
        # Single list for all alleles,
        # ref is always position 0
        # e.g., biallelic SNP has length 2
        # e.g., A/T SNP is ['A', 'T']
        self.alleles = list()
        self.alleles.append(ref_allele)
        for alt in alt_alleles:
            self.alleles.append(alt)
        # Set the different types of variants
        self.type = None
        allele_lens = [ len(a) for a in self.alleles ]
        ref_len = allele_lens[0]
        same_lens = None
        ref_larger = None
        ref_smaller = None
        for alt_len in allele_lens[1:]:
            if alt_len == ref_len:
                same_lens = True
                ...
                ref_smaller = True
        if same_lens and ref_len==1:
            self.type = 'snp'
        elif same_lens and ref_len>1:
            self.type = 'mnp'
        elif ref_smaller and not ref_larger and ref_len==1:
            self.type = 'insertion'
        elif ref_larger and not ref_smaller and ref_len>1:
            self.type = 'deletion'
        elif ref_larger and ref_smaller:
            # For really crazy cases...
            self.type = 'complex'
    def get_genotype(self, sample_i, allele_n, ploidy=2) -> int:
        '''
        Function to extract specific genotype from 'geno' list
        Args:
            sample_i: (int) Index of the sample on the list (0-based)
            allele_i: (int) Index of the allele being processed (0-based)
            ploidy: (int) Ploidy, i.e., number of possible alleles per sample
        Returns:
            genotype: (int) Specific genotype for that sample allele
        '''
        assert allele_n in range(ploidy)
        genotype = int(self.geno[ (sample_i * ploidy) + allele_n ])
        return genotype
    def get_allele(self, genotype_idx) -> str:
        '''
        Extract the allele (nucleotide sequence) of a given genotype index.
        Args:
            genotype_idx: (int) Numeric index of the genotype
        Returns:
            allele_seq: (str) Sequence of that genotype
        '''
        if genotype_idx is None:
            return 'N'
        else:
            assert genotype_idx in range(len(self.alleles))
            return self.alleles[genotype_idx]
    def __str__(self):
        geno_str = ''
        for g in self.geno:
            if g is None:
                ...
                geno_str += str(g)
        alleles_str = ','.join(self.alleles)
        return f'{self.chr} {self.pos} {alleles_str} {self.type} {geno_str}'
...
def main():
    ...
    # Load and filter the variants
    variants = load_variants(args.vcf, annotations, args.snps_only)
    # # Process all samples
    # process_all_samples(samples, annotations, genome, args.vcf, args.out_dir, args.threads)
    # Done!
